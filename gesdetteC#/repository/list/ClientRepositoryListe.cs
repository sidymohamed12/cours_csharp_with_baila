...
namespace gesdette.repository.list
{
    public class ClientRepositoryListe : RepositoryListeImplement<Client>, IClientRepository
    {
        ...
        {
            return listes.FirstOrDefault(client => client?.Surnom?.Equals(name, StringComparison.Ordinal) == true);
        }
        ...
        {
            var existingClient = listes.FirstOrDefault(cl => cl?.Telephone?.Equals(client.Telephone) == true);
